...
const AdminDashboard = ({ schoolId }) => {
  ...
  const handleRegisterStaff = async (e) => {
    ...
    const { error: insertError } = await supabase.from('staff').insert([{
      full_name: fullName, email: email, stored_password: password, role: role, ic_number: cleanIcNumber, phone_no: phoneNo, school_id: schoolId, profile_pic: finalProfilePicUrl 
    }]);
    ...
    if (insertError) {
      ...
      setFullName(''); setEmail(''); setPassword(''); setIcNumber(''); setPhoneNo(''); setAvatarFile(null); 
    ...
    <div className="max-w-5xl mx-auto space-y-6 fade-in">
      ...
      {activeTab === 'manage' && (
        <div className="bg-white p-8 rounded-xl shadow border border-slate-200 fade-in">
          <h2 className="text-xl font-bold text-slate-800 mb-4">Institutional Roster Directory</h2>
          ...
          <div className="overflow-x-auto rounded-lg border border-slate-200">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 text-sm">
                {staffList.length === 0 ? (
                  <tr><td colSpan="4" className="p-8 text-center text-slate-400">No staff members provisioned.</td></tr>
                ) : (
                  staffList.map((staff) => (
                    <tr key={staff.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="p-4 align-middle text-right space-x-3">
                        <button onClick={() => triggerEditMode(staff)} className="text-xl hover:scale-110 transition-transform" title="Edit Profile">
                          ✏️
                        </button>
                        <button onClick={() => handleDeleteStaff(staff.id, staff.full_name)} className="text-xl hover:scale-110 transition-transform" title="Delete Profile">
                          🗑️
                        </button>
                      </td>
                    </tr>
        ...
        <div className="bg-white p-8 rounded-xl shadow border border-slate-200 fade-in max-w-2xl">
          ...
          <form onSubmit={handleRegisterStaff} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Full Name</label><input type="text" value={fullName} onChange={e => setFullName(e.target.value)} className="border p-2 w-full rounded focus:ring-2 focus:ring-teal-500" required /></div>
              <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Official Email</label><input type="email" value={email} onChange={e => setEmail(e.target.value)} className="border p-2 w-full rounded focus:ring-2 focus:ring-teal-500" required /></div>
              <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Phone Number</label><input type="tel" value={phoneNo} onChange={e => setPhoneNo(e.target.value)} className="border p-2 w-full rounded focus:ring-2 focus:ring-teal-500" required /></div>
            </div>
            <div className="grid grid-cols-2 gap-4 items-start">
              <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">IC Number</label><input type="text" value={icNumber} onChange={e => setIcNumber(e.target.value)} className="border p-2 w-full rounded focus:ring-2 focus:ring-teal-500" required /></div>
              <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Profile Picture</label><input id="avatarInput" type="file" accept="image/jpeg, image/png" onChange={e => setAvatarFile(e.target.files[0])} className="border p-1.5 w-full rounded text-sm" /></div>
            </div>
      ...
      {activeTab === 'edit' && editingStaff && (
        <div className="bg-white p-8 rounded-xl shadow border-t-4 border-blue-500 fade-in max-w-2xl">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h2 className="text-2xl font-bold text-slate-800">Update Profile: {editingStaff.full_name}</h2>
              <p className="text-sm text-slate-500">Modify system credentials and details.</p>
            </div>
            <button onClick={() => setActiveTab('manage')} className="text-slate-400 hover:text-slate-700 font-bold text-xl">✕</button>
          </div>
          ...
          <form onSubmit={handleUpdateStaff} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" value={editingStaff.full_name} onChange={e => setEditingStaff({...editingStaff, full_name: e.target.value})} className="border p-2 w-full rounded focus:ring-2 focus:ring-blue-500" required />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Workspace Scope</label>
                <select value={editingStaff.role} onChange={e => setEditingStaff({...editingStaff, role: e.target.value})} className="border p-2 w-full rounded bg-slate-50 font-medium text-slate-700">
                  <option value="standard_teacher">Standard Classroom Teacher</option>
                  <option value="asset_teacher">Authorized Asset Field Teacher</option>
                </select>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="email" value={editingStaff.email} onChange={e => setEditingStaff({...editingStaff, email: e.target.value})} className="border p-2 w-full rounded focus:ring-2 focus:ring-blue-500" required />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Phone Number</label>
                <input type="tel" value={editingStaff.phone_no} onChange={e => setEditingStaff({...editingStaff, phone_no: e.target.value})} className="border p-2 w-full rounded focus:ring-2 focus:ring-blue-500" required />
              </div>
            </div>

            <div className="mt-6 pt-4 border-t border-slate-100">
              <label className="block text-xs font-bold text-slate-500 uppercase mb-1 text-red-500">System Password</label>
              <input type="text" value={editingStaff.stored_password} onChange={e => setEditingStaff({...editingStaff, stored_password: e.target.value})} className="border p-2 w-full rounded focus:ring-2 focus:ring-red-400 bg-red-50" required />
            </div>
