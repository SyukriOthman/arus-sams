...
const UserProfile = ({ session, onSessionUpdate }) => {
  ...
  return (
    <div className="max-w-3xl mx-auto bg-white p-8 rounded-xl shadow border border-slate-200 fade-in">
        ...
        <div className="flex-1">
          ...
          <span className={`inline-block mt-2 px-3 py-1 rounded-full text-xs font-bold capitalize 
            ${profile.role === 'superadmin' ? 'bg-purple-100 text-purple-700' : 
              profile.role === 'headmaster' ? 'bg-amber-100 text-amber-700' : 'bg-teal-100 text-teal-700'}`}>
      ...
      {isEditing && (
        <form onSubmit={handleUpdateAvatar} className="mb-8 bg-slate-50 p-6 rounded-lg border border-slate-200 fade-in">
